...
void MyClass::start()
{
	...
	Mat temp = imread("..\\resultshow.bmp");//所有图片处理后保存的路径用于显示
	Mat temp1;
...
void MyClass::shotimage()
{
	...
	else
	{
		...
		Point3f pt_temp = Point3f(0, 0, 0);//保存截取图片的初始位置，方便恢复在原始图片中位置
		Point3f *pt = &pt_temp;
		pair<IplImage*, Point3f*> *temp = new pair<IplImage*, Point3f*>(srcImg, pt);//传入鼠标函数中的数据
		Point2f pt1;
		vector<vector<string>> results;
		...
		if ((*pt).z != 0)
		{
			pt1.x = (*pt).x;
			pt1.y = (*pt).y;
			procfunc("..\\shot.bmp",path, pt1,results);//截取后保存的图片路径
